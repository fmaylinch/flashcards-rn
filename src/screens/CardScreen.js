...
const CardScreen = ({navigation}) => {
  ...
  function japaneseView() {
    ...
    // Generated by GPT4
    function wordsContainedInArray(s, arr) {
      let result = [];
  
      let currentIndex = 0;
  
      for (let i = 0; i < arr.length; i++) {
          let splitIndex = s.indexOf(arr[i], currentIndex);
  
          if (splitIndex !== -1) {
              if (splitIndex > currentIndex) {
                  result.push({word: s.substring(currentIndex, splitIndex), inArray: false});
              }
              result.push({word: arr[i], inArray: true});
              currentIndex = splitIndex + arr[i].length;
          }
      }
  
      if (currentIndex < s.length) {
          result.push({word: s.substring(currentIndex), inArray: false});
      }
    ...
    function textViewFor(part) {
      if (part.inArray) {
        return <Text style={styles.textMainPart} onPress={() => openNihongo(part.word)}>{part.word}</Text>;
      } else {
    ...
    const parts = wordsContainedInArray(card.front, card.mainWords);
  ...
  // TODO - If card.mainWords has words, create individual links.
  //        Create a View with Texts in a row.
